...
This sparse-clones `data/` from the RSEC repo (~90 s), parses every `*.biotools.json` file, and writes `shelley_bio/data/rsec_meta.json.gz`.

```bash
# Force tarball download instead of sparse clone:
shelley-bio-build-rsec --method tarball

# Check field-coverage statistics without writing anything:
shelley-bio-build-rsec --assess

# Fetch a specific branch/tag:
shelley-bio-build-rsec --ref main
```
...
```bash
uv run python shelley_bio/scripts/assess_coverage.py           # both sources
uv run python shelley_bio/scripts/assess_coverage.py rsec
uv run python shelley_bio/scripts/assess_coverage.py toolfinder
```
...
```bash
uv run python shelley_bio/scripts/assess_regression_tools.py
```

## Refresh a guts base-image manifest

The `shelley_bio/data/guts_db/` directory holds Singularity manifests used to subtract conda infrastructure from tool containers. Refresh when a base image's conda version makes a major jump.
...
```bash
uv run python shelley_bio/cache/build_cache.py
```
